/**
 * Nota Credito de factura electronica de salud.
 * Referencia siempre a una FacturaSalud existente.
 * Tabla: notas_credito_salud
 */
import {
...
export type NotaCreditoSaludStatus =
  | 'borrador'
  | 'enviada'
  | 'aprobada'
  | 'aceptada'
...
@Entity('notas_credito_salud')
@Unique(['company_id', 'nota_number'])
export class NotaCreditoSalud {
  ...
  /** RESTRICT: no se puede eliminar una factura que tenga notas credito.
  ...
  /** Numero completo de la nota credito (prefix + number). Unico por empresa. */
  @Column({ length: 50 })
  nota_number!: string;

  @Column({ length: 20, default: 'NCSS' })
  prefix!: string;
