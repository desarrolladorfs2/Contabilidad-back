/**
 * Carga (o actualiza) el catálogo de códigos CIE-10 en la tabla
 * cat_codigos_cie10, a partir del archivo que compartió el cliente
 * (Bd_Cargues/TablaReferencia_CIE10__1.xlsx), ya limpio y convertido a CSV
 * en backend/src/seeds/data/cie10.csv (sin espacios dobles ni al inicio/
 * final en los nombres — el archivo original sí los tenía).
 *
 * Idempotente: se puede correr varias veces sin duplicar — si el código
 * ya existe, actualiza su nombre y descripción; si no existe, lo crea.
 *
 ...
 *
 * Uso: npx ts-node src/scripts/import-cie10.ts
 */
...
const CSV_PATH = path.join(__dirname, '..', 'seeds', 'data', 'cie10.csv');
 ...
 * cuando se arranca el backend normal. Se replica aquí (igual que en
 * import-unspsc.ts) para no perder esa protección justo en el script que va
 * a crear/llenar una tabla nueva.
 */
function backupSqliteIfApplicable(): void {
  ...
  const dst = `${dbPath}.backup-${ts}-pre-import-cie10`;
  fs.copyFileSync(dbPath, dst);
  console.log(`[import-cie10] Respaldo creado antes de importar: ${path.basename(dst)}`);
}
...
async function main() {
  if (!fs.existsSync(CSV_PATH)) {
    throw new Error(`No se encontró el archivo ${CSV_PATH}. Corre este script desde el backend con el archivo cie10.csv presente.`);
  }
  backupSqliteIfApplicable();
  console.log('[import-cie10] Leyendo CSV...');
  const rows = parseCsv(fs.readFileSync(CSV_PATH, 'utf-8'));
  console.log(`[import-cie10] ${rows.length} códigos leídos del CSV.`);
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    const aInsertar: Partial<CodigoCie10>[] = [];
    const aActualizar: Partial<CodigoCie10>[] = [];

    for (const r of lote) {
      const data: Partial<CodigoCie10> = {
        codigo:      r['codigo'],
        nombre:      r['nombre'],
    ...
    if (aInsertar.length) {
      await repo.insert(aInsertar as CodigoCie10[]);
    ...
    if ((i / BATCH) % 20 === 0) {
      console.log(`[import-cie10] Progreso: ${Math.min(i + BATCH, rows.length)}/${rows.length}`);
    }
  }

  console.log(`[import-cie10] Completado: ${creados} código(s) nuevo(s) creado(s), ${actualizados} actualizado(s).`);
  const total = await repo.count();
  console.log(`[import-cie10] Total de códigos en cat_codigos_cie10: ${total}`);
